<?php
	if ( post_password_required() ) {
		echo '<h3 class="comments-header">' . __( 'Contenido Protegido', 'buddypress' ) . '</h3>';
		echo '<p class="alert password-protected">' . __( 'Ingrese para ver los comentartios.', 'buddypress' ) . '</p>';
		return;
	}
...
<?php else : ?>
	...
	<?php if ( pings_open() && !comments_open() && ( is_single() || is_page() ) ) : ?>
		<p class="comments-closed pings-open">
			<?php printf( __( 'Comments are closed, but <a href="%1$s" title="Direcion fija para este post">trackbacks</a> and pingbacks are open.', 'buddypress' ), trackback_url( '0' ) ) ?>
		</p>
	<?php elseif ( !comments_open() && ( is_single() || is_page() ) ) : ?>
		<p class="comments-closed">
			<?php _e( 'Comentarios deshabilitados.', 'buddypress' ) ?>
		</p>
	<?php endif; ?>
